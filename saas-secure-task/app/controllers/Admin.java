...
public class Admin extends Controller {
    ...
    public static Result profile() {
        //todo
        Form<ChangePassword> canForm = Form.form(ChangePassword.class);
        return ok(profile.render(canForm));
    }
    ...
    public static Result changePassword() {
        //todo
        Form<ChangePassword> canForm = Form.form(ChangePassword.class).bindFromRequest();
        if (canForm.hasErrors())
            return badRequest(profile.render(canForm));
        else {
            flash("success","Пароль успешно изменен");
            Form<ChangePassword> canForm1 = Form.form(ChangePassword.class);
            return ok(profile.render(canForm1));
        }
    }
}